...
spot. Conventionally, we then multiply by a million (the scale.factor parameter), which yields 
...
Since we expect most genes to have only technical (and also biological) variance, the trend line should
...
Yet, in both, there remains an evident trend in the plot between mean and variance. Let's next look at
...
highest end of the expression. Note that all of the highly variable genes are peaked
in a narrow range of high expression values.

Let's look at the spatial expression of the two most variable genes.
...
Let's see if we can improve on the mean-variance trend exhibited for highly expressed genes
under log normalization. We will apply [`SCTransform`](https://satijalab.org/seurat/articles/sctransform_vignette.html),
...
it refers to the variance in the Pearson residuals, similar to the 
Each point shows one gene. By default, 
