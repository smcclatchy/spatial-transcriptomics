...
We selected one sample from each subject (`151507`, `151669`, and `151673`), as detailed in the referenced study ([Maynard et al, Nat Neurosci 24, 425–436 (2021)](https://doi.org/10.1038/s41593-020-00787-0)). This selection allows us to analyze the consistency of differential expression across individuals while accounting for subject-specific batch effects.
...
for (i in selected_samples) {
  pseudobulk_seurat <- AggregateExpression(objs[[i]], 
                                           assays = 'Spatial', 
                                           return.seurat = TRUE, 
                                           group.by = 'layer_guess')
  pseudobulk_seurat@meta.data$sample <- names(objs[i])
...
``` error
Error in eval(expr, envir, enclos): object 'objs' not found
```
...
for (sample_id in selected_samples) {
  p1 <- plot_sample_expression(objs[[sample_id]], "MOBP", sample_id)
  p2 <- plot_sample_expression(objs[[sample_id]], "MBP" , sample_id)
  p3 <- plot_layer_expression(objs[[sample_id]], sample_id)
  combined <- p1 + p2 + p3
