...
![Tissue Clustering](fig/tissue_cluster_resol.png){alt.text="Tissue Clustering"}
...
![UMAP Clustering](fig/umap_cluster_resol.png){alt.text="UMAP Clustering"}
...
plot in the middle row has nine clusters, which is alos close to the publication.
...
```{r}
SpatialDimPlot(filter_st, group.by = "seurat_clusters")
```
...
```{r}
HVG_selection_methods <- c("vst", "mean.var.plot")
Cluster_resolutions <- c(0.5, 1.0, 1.5)
PCA_components_list <- c(5, 10, 25) # Updated to list to avoid confusion with the loop variable
genes_props <- c(0.1, 0.5, 1)

# Initialize a dataframe to store the results
results_df <- data.frame(HVG_selection_method = character(),
                         Cluster_resolution = numeric(),
...
for (method in HVG_selection_methods) {
  filter_st <- FindVariableFeatures(filter_st, assay='Spatial', 
                                    selection.method = method, 
                                    nfeatures = 2000)
  for (resolution in Cluster_resolutions) {
    for (PCA_components in PCA_components_list) {
      for (genes_prop in genes_props) {
        
        feats_used <- head(VariableFeatures(filter_st), round(length(VariableFeatures(filter_st)) * genes_prop))
        filter_st <- ScaleData(filter_st, features = feats_used)
        filter_st <- RunPCA(filter_st, features = feats_used, npcs = PCA_components)
        filter_st <- FindNeighbors(filter_st, reduction = "pca", dims = 1:PCA_components)
        temp_clust <- paste("clusters_", method, "_", resolution, "_", PCA_components, "_", genes_prop, sep="")
        filter_st <- FindClusters(filter_st, reduction = "pca", resolution=resolution, cluster.name = temp_clust)

        ari <- adjustedRandIndex(filter_st@meta.data$layer_guess, filter_st@meta.data[[temp_clust]])
        
        results_df <- rbind(results_df, data.frame(HVG_selection_method = method,
                                                   Cluster_resolution = resolution,
                                                   PCA_components = PCA_components,
                                                   Genes_prop = genes_prop,
                                                   ARI = ari))
      }
    }
  }
}
write.csv(results_df, file = 'SCT_clustering_results.csv')
```
