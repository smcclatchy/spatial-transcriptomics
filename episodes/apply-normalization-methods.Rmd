...
It has also added new Assay called "SCT".
...
```{r}
filter_st@active.assay = "Spatial"
...
```{r}
counts_log <- LayerData(filter_st, layer = "data")
hist(colSums2(counts_log), main = "Log-norm")

filter_st@active.assay = "SCT"
...
```{r}
filter_st@active.assay = "Spatial"
top15 <- head(VariableFeatures(filter_st), 15)
plot1 <- VariableFeaturePlot(filter_st, log = NULL) + 
...
```{r echo=FALSE,warning=FALSE,message=FALSE}
# Save data for build system to use in next lesson.
