...
filter_st = readRDS(file = 'filter_st2.rds')
```
...
```{r}
sct <- SCTransform(filter_st, 
                   assay = "Spatial")
```

The SCTransform method added the normalized counts (called "data") and the 
standardized counts (called "scale.data") to the Seurat object. 
...
```{r}
VariableFeaturePlot(sct, log = NULL) + 
...
```{r}
log_norm <- NormalizeData(filter_st, 
                          assay                = "Spatial", 
...
The Log Normalization adds a "data" object to the Seurat object.
...
```{r}
log_norm <- FindVariableFeatures(log_norm,  
                                 assay            = 'Spatial', 
                                 selection.method = "mean.var.plot",
                                 nfeatures        = 3000)
VariableFeaturePlot(log_norm, log = NULL) + 
...
```{r}
counts_log <- LayerData(log_norm, layer = "data")
hist(colSums2(counts_log), main = "Log-norm")

counts_sct <- LayerData(sct, layer = "data")
...
```{r}
top15 <- head(VariableFeatures(log_norm), 15)
plot1 <- VariableFeaturePlot(log_norm, log = NULL) + 
           ggtitle("Variable Features - Log normalization")
plot1 <- LabelPoints(plot = plot1, points = top15, repel = TRUE)

top15SCT <- head(VariableFeatures(sct), 15)
plot2    <- VariableFeaturePlot(sct) + 
              ggtitle("Variable Features - SCT")
...
### Applying the Selected Normalization

Since we have selected the SCT transformation, apply it to our Seurat object
now.
...
saveRDS(filter_st, file = 'filter_st3.rds')
```
