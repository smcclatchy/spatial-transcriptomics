...
> DMG: Should we work with the filtered file? Or start with the raw file and filter the spots?
...
genes across the tissue. This can be helpful in identifying problems with the
We will use Seurat's 
[SpatialFeaturePlot](https://satijalab.org/seurat/reference/spatialplot) 
function to look at these values.


Brain: we may have layers of different kinds of cells.
e.g. Cancer: stromal cells have lower counts than tumor cells.
...
SpatialFeaturePlot(filter_st, features = "nCount_Spatial")
```
...
SpatialFeaturePlot(filter_st, features = "nFeature_Spatial")
```
