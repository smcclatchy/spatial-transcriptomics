...
source("https://github.com/smcclatchy/spatial-transcriptomics/blob/main/code/spatial_utils.R")
```
...
```{r fig.alt = "Boxplot showing lower trancript counts in background area of slide", fig.cap = "UMI Counts in Tissue and Background"}
raw_st@meta.data %>%
...
```{r fig.alt = "Figure showing UMI counts in each spot with varying intensity across the tissue",fig.cap="UMI Counts in each Spot"}
plot1 <- SpatialDimPlot(filter_st, alpha = c(0, 0)) + 
           NoLegend()
...
save_seurat_object(obj = filter_st, file_prefix = 'datapreproc')
```
